...
public class CD {
	...
	public int compareToArtiste(CD cd){
		int res = 0;
		if(this.nomArtiste.compareTo(cd.nomArtiste)>0){
			res = 1;
		} else if (this.nomArtiste.compareTo(cd.nomArtiste)<0) {
			res = -1;
		}
		return res;
	}

	public int compareToNbPiste(CD cd){
		int res = 0;
		if(this.pistes.size() > cd.getNbPiste()){
			res = 1;
		}else if (this.pistes.size() < cd.getNbPiste()){
			res = -1;
		}
		return res;
	}
